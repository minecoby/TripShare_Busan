  ...
  class _SummationPageState extends State<SummationPage> {
    ...
    Future<void> _loadPost() async {
      ...
      print("토큰입니다: $token");
      print(_postSummaryController.post?.routeId);

      // 1. 로그인 안 한 경우
      if (token == null) {
        errormessage = "🔒 루트 정보는 로그인 후 확인할 수 있습니다.\n로그인 후 다시 확인해주세요!";
      }
      // 2. 로그인 했는데 routeId가 없는 경우
      else if (_postSummaryController.post?.routeId == null) {
        errormessage = "루트 정보가 등록되지 않았습니다.";
      }
      // 3. 로그인도 했고, routeId도 있는 경우
      else {
        try {
