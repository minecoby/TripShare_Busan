...
class MapPage extends StatefulWidget {
  ...
  const MapPage({super.key, this.initialKeyword});
...
class _MapPageState extends State<MapPage> {
  ...
  Future<void> _searchPlace(String query) async {
    ...
    final response = await http.get(
      url,
      headers: {'Authorization': 'KakaoAK ${dotenv.env['KAKAO_REST_KEY']}'},
    );
        ...
        for (var doc in documents.take(30)) {
          ...
          marker.setOnTapListener((overlay) {
            ...
            final index = _searchResults.indexWhere(
              (p) =>
                  p['lat'] == tappedPlace['lat'] &&
                  p['lng'] == tappedPlace['lng'],
            );
