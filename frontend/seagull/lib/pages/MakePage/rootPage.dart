...
class _AnimatedPlaceTimelineState extends State<AnimatedPlaceTimeline>
  ...
  List<File?> markerImages = [];
  Future<void> _pickImage(int index) async {
    final picker = ImagePicker();
    final pickedFile = await picker.pickImage(source: ImageSource.gallery);
    if (pickedFile != null) {
      setState(() {
        markerImages[index] = File(pickedFile.path);
      });
    }
  }
  ...
  void initState() {
    super.initState();
    markerImages = List<File?>.filled(widget.num, null);
  ...
  void didUpdateWidget(covariant AnimatedPlaceTimeline oldWidget) {
    ...
    if (oldWidget.num != widget.num) {
      final oldTitleControllers = titleControllers;
      final oldAddressControllers = addressControllers;
      final oldAnimControllers = controllers;

      markerImages = List<File?>.generate(widget.num, (i) {
        return i < markerImages.length ? markerImages[i] : null;
      });

      titleControllers = List.generate(widget.num, (i) {
        return i < oldTitleControllers.length
            ? oldTitleControllers[i]
            : TextEditingController();
      });

      addressControllers = List.generate(widget.num, (i) {
